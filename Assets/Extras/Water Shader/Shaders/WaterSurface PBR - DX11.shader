        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    ...
                    float node_1302 = length((v.texcoord0*2.0+-1.0));
                    float2 node_7065 = (v.texcoord0*(-1.0));
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (node_3745.rg*(_RefractionIntensity*0.1+0.0));
...
///////// Gloss:
                float node_2784 = saturate((sceneZ-partZ)/_FoamDistance);
                float gloss = clamp((node_2784*_Gloss),0.1,1);
...
////// Specular:
                ...
                float3 node_221 = clamp((node_4593*(1.0 - node_2784)*_FoamIntensity),0,1);
                float3 specularColor = (_Specularity*pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel)*(1.0 - node_221));
...
/////// Diffuse:
                ...
                float node_9504 = (-0.1);
                float node_3478 = 1.0;
                float3 diffuseColor = (saturate((pow(saturate((node_3478 + ( (((sceneZ-partZ)*_Density) - node_9504) * (0.0 - node_3478) ) / ((_WaterColor.rgb*6.28318530718) - node_9504))),_FadeLevel)+(1.0 - saturate((sceneZ-partZ)/_ShoreTransparency))))*sceneColor.rgb);
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(lerp(sceneColor.rgb, finalColor,saturate((saturate((sceneZ-partZ)/_ShoreOpacity)*_WaterOpacity))),1);
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor * saturate((saturate((sceneZ-partZ)/_ShoreOpacity)*_WaterOpacity)),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
