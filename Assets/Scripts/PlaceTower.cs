...
public class PlaceTower : MonoBehaviour {
	...
	public void Destruir(GameObject tower){
		...
		if(torre != null){																				//se o local tiver algum monstro
			...
			gameManager.Gold += (int)(tropas * 0.4);
	...
	private void BotarTorre(GameObject tower){
		if(canPlaceTower (tower)) {																		//se puder botar monstro
			...
			gameManager.Gold -= torre.GetComponent <TowerData> ().CurrentLevel.tropas;					//acessa a carteira do jogador e reduz o seu dinheiro, de acordo com o custo de compra do monstro.
			...
			audiosource.PlayOneShot (audiosource.clip);													//toca o som de botar item
			gameManager.Gold -= torre.GetComponent <TowerData> ().CurrentLevel.tropas;					//acessa a carteira do jogador e reduz o seu dinheiro, de acordo com o custo de melhoria do monstro.
		}
	}
	...
	private bool canUpgradeTower() {																	//procedimento que vai informar se pode evoluir um monstro;
		if(torre != null){																				//se o local tiver algum monstro
			...
			if(nextLevel != null){																		//se existir um proximo level
				return gameManager.Gold >= nextLevel.tropas;											//vai retornar verdadeiro se o dinheiro atual for maior que o custo de evoluir para o proximo level.
			}
		}
